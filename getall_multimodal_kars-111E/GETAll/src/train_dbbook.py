...
def train(train_ratings: pd.DataFrame, batch_size: int, epochs: int, device: str, model_class: Type[GETAllNetworkConcat],
...
@torch.no_grad()
def test(model: GETAllNetworkConcat, test_ratings: pd.DataFrame, batch_size: int, device: str, dropout_value: float):
...
if __name__ == "__main__":
    ...
    feature2 = dict(sorted((pkl.load(open('C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/graph/compgcn_384_n1.pkl', 'rb'))).items()))
    ...
    feature_list = [ ['user-centroid_item_emb', 'compgcn_384_n1'] ]
