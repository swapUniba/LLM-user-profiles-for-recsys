...
class FiveSourceGETAllNetwork(GETAllNetwork):
    ...
    def __init__(self, modality_features, dropout_value):
        ...
        if len(self.modality_features) != 5:
            raise ValueError("This class only supports four sources of features\n"
        ...
        self.fourth_modality_features = torch.nn.Parameter(self.modality_features[3].float(), requires_grad=False)
        self.fifth_modality_features = torch.nn.Parameter(self.modality_features[4].float(), requires_grad=False)

        self.allowed_dims = [4096, 2048, 1024, 768, 512, 384, 192, 128, 100]
        allowed_dims = [4096, 2048, 1024, 768, 512, 384, 256, 192, 128, 100]
        ...
        starting_dim = self.fourth_modality_features.size(1)
        if starting_dim not in allowed_dims:
            raise ValueError(f'Embedding dimensionality not supported: passed {starting_dim},\n'
                f' possible dims:{set(self.allowed_dims.keys())}')

        starting_dim = self.fifth_modality_features.size(1)
        if starting_dim not in allowed_dims:
            raise ValueError(f'Embedding dimensionality not supported: passed {starting_dim},\n'
                f' possible dims:{set(self.allowed_dims.keys())}')
        ...
        self.fourth_dense_user = self.create_dense_entity_layers(self.fourth_modality_features.size(1), dropout_value)
        self.fourth_dense_item = self.create_dense_entity_layers(self.fourth_modality_features.size(1), dropout_value)

        self.fifth_dense_user = self.create_dense_entity_layers(self.fifth_modality_features.size(1), dropout_value)
        self.fifth_dense_item = self.create_dense_entity_layers(self.fifth_modality_features.size(1), dropout_value)


# simple concat
class GETAllNetworkFiveConcat(FiveSourceGETAllNetwork):
    ...
    def forward(self, x):
        ...
        fourth_users_features = self.fourth_modality_features[user_idx]
        fourth_items_features = self.fourth_modality_features[item_idx]

        fifth_users_features = self.fifth_modality_features[user_idx]
        fifth_items_features = self.fifth_modality_features[item_idx]
        ...
        fourth_users_features = self.fourth_dense_user(fourth_users_features)
        fourth_items_features = self.fourth_dense_item(fourth_items_features)

        fifth_users_features = self.fifth_dense_user(fifth_users_features)
        fifth_items_features = self.fifth_dense_item(fifth_items_features)

        # concat users and items and reduce dimensionalities
        user = self.dense_user(torch.cat([first_users_features, second_users_features, third_users_features, fourth_users_features, fifth_users_features], dim=-1))
        item = self.dense_item(torch.cat([first_items_features, second_items_features, third_items_features, fourth_items_features, fifth_items_features], dim=-1))
...
class GETAllNetworkFiveSelfSourcesCross(FiveSourceGETAllNetwork):
    ...
    def __init__(self, modality_features, dropout_value):
        ...
        # query, key, value for video embeddings
        self.video_qkv_1 = nn.Linear(256, 3*256)

        # query, key, value for audio embeddings
        self.audio_qkv_1 = nn.Linear(256, 3*256)
        ...
        # linear layers for weighted_video
        self.linear_weighted_video = torch.nn.Sequential(
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU()
        ...
        # linear layers for weighted_audio
        self.linear_weighted_audio = torch.nn.Sequential(
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU()
        ...
        self.linear_query = torch.nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        # fourth self attention on video embeddings
        query, key, value = self.video_qkv_1(torch.stack([fourth_users_features, fourth_items_features], dim=1)).chunk(3, dim=-1)
        weighted_video = self.compute_attention(query, key, value).sum(dim=1)

        # fifth self attention on audio embeddings
        query, key, value = self.audio_qkv_1(torch.stack([fifth_users_features, fifth_items_features], dim=1)).chunk(3, dim=-1)
        weighted_audio = self.compute_attention(query, key, value).sum(dim=1)
        ...
        weighted_video = self.linear_weighted_video(weighted_video)
        weighted_audio = self.linear_weighted_audio(weighted_audio)

        # concat and linear layers for key and value based on text and img
        linear_query = self.linear_query(torch.concat([weighted_graph, weighted_text], dim=-1))
        linear_key_value = self.linear_key_value(torch.concat([weighted_img, weighted_video, weighted_audio], dim=-1))

        # cross attention with graph as query, and projected concat as key value
        query = self.final_q(linear_query)
