...
class FourSourceGETAllNetwork(GETAllNetwork):
    ...
    def __init__(self, modality_features, dropout_value):
        ...
        if len(self.modality_features) != 4:
        ...
        all_dims = [4096, 2048, 1024, 768, 512, 384, 256, 192, 128, 100]

        starting_dim = self.first_modality_features.size(1)
        if starting_dim not in all_dims:
...
# simple concat
class GETAllNetworkFourConcat(FourSourceGETAllNetwork):
    ...
    def forward(self, x):
        ...
        user = self.dense_user(torch.cat([first_users_features, second_users_features, third_users_features, fourth_users_features], dim=-1))
        item = self.dense_item(torch.cat([first_items_features, second_items_features, third_items_features, fourth_items_features], dim=-1))
...
class GETAllNetworkFourSelfSourcesCross(FourSourceGETAllNetwork):
    ...
    def forward(self, x):
        ...
        linear_key_value = self.linear_key_value(torch.concat([weighted_img, weighted_video], dim=-1))
