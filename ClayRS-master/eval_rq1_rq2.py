...
def eval(dataset='', prediction_type_model='', ks=[10], relevant_threshold=1):

    if prediction_type_model == 'baseline':
        preds_fold = f'rq1_baseline/{dataset}'
        print("Current working directory:", os.getcwd())
        print("Predictions folder path:", preds_fold)
        prediction_dest_results = 'rq1'
    else:
        preds_fold = f'rq2_getall/{dataset}'
        print("Current working directory:", os.getcwd())
        print("Predictions folder path:", preds_fold)
...
for dataset in ['dbbook', 'ml1m']:
    for prediction_type_model in ['getall', 'baselines']:
        ...
        eval(dataset, prediction_type_model, ks=[10], relevant_threshold=1)
