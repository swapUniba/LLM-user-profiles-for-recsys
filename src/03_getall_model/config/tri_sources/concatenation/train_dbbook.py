...
def train(train_ratings: pd.DataFrame, batch_size: int, epochs: int, device: str, model_class: Type[GETAllNetworkTriConcat],
           modality_features: np.array, weights: list, dropout_value: float):

    train_ds = GETAllDataset(train_ratings)
    #Utilizza un DataLoader per suddividere i dati in batch.
...
@torch.no_grad()
def test(model: GETAllNetworkTriConcat, test_ratings: pd.DataFrame, batch_size: int, device: str, dropout_value: float):
...
if __name__ == "__main__":
    getall_network_class = GETAllNetworkTriConcat
    ...
    emb_folders = ['C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/images/',
                'C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/text/',
                'C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/graph/',
                ]
    ...
    feature1 = dict(sorted((pkl.load(open('C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/images/user-centroid_item_emb.pkl', 'rb'))).items()))
    feature2 = dict(sorted((pkl.load(open('C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/text/user-profile_item_emb.pkl', 'rb'))).items()))
    feature3 = dict(sorted((pkl.load(open('C:/Users/arian/Downloads/getall_multimodal_kars-111E/GETAll/data/embeddings/dbbook/graph/compgcn_384_n1.pkl', 'rb'))).items()))


    source_features = {
        key: (torch.from_numpy(feature1[key]), torch.from_numpy(feature2[key]), torch.from_numpy(feature3[key])) 
        for key in feature1.keys() & feature2.keys() & feature3.keys()
    }
    ...
    graph_tensor = torch.tensor(np.array(list(feature3.values())), dtype=torch.float32)

    model = GETAllNetworkTriConcat(
        modality_features = [text_tensor, image_tensor, graph_tensor],  
        dropout_value=0.2 
    ).to(device)


    feature_list = [ ['user-centroid_item_emb', 'user-profile_item_emb', 'compgcn_384_n1'] ]
    getall_models = [GETAllNetworkTriConcat]    
    dropout_values = [ 0.2]
    setting_names = []
